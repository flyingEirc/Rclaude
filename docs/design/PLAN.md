...
远端存在一个执行环境，用户通过对话或任务调度驱动该执行环境操作**用户本地机器上的文件**。执行侧的工具是固定的 bash/shell 命令（`cat`、`sed`、`grep`、`ls` 等），无法修改其工具调用方式。

### 1.2 关键约束

- 执行侧通过 bash/powershell 调用文件命令，调用方式不可更改
- 执行侧认为文件在本地文件系统上，直接通过路径执行 shell 命令
- 文件实际存储在每个用户的本地机器上
- 用户规模：1-20 人小团队
...
```
用户对话 / 任务触发 → Server 端执行环境决定操作文件
                ↓
    执行环境运行 bash: cat /workspace/user1/main.go
                ↓
    Server 上该路径必须是一个"真实文件"
    但实际内容来自用户本地机器
                ↓
    结果返回给执行环境 → 后续流程继续处理 → 回复用户
```
...
3. **Server 端执行环境**（运行在 Server 同一台或同网络机器，通过 bash/powershell 访问文件）
...
5. 执行环境执行 `cat /workspace/user1/main.go` → 内核触发 FUSE Read 回调
6. FUSE handler 通过 gRPC stream 向对应用户的 Daemon 请求文件内容
7. Daemon 读取本地真实文件，回传内容
8. FUSE 返回内容给 cat 进程，执行环境拿到结果
...
- 文件大小限制：单文件读取上限（如 10MB），防止执行环境读取超大文件
...
**为什么必须用 FUSE：**
Server 端执行环境通过 bash 命令操作文件，当它执行 `cat /workspace/user1/main.go` 时，内核需要有一个真实的文件系统响应这个 read 系统调用。FUSE 允许在用户空间实现文件系统，拦截内核的 VFS 调用，转发为网络请求。
...
执行环境通常会连续读取多个文件（比如先 ls 看目录，再 cat 几个文件）。可以做简单的预取：当 Readdir 被调用时，异步预取该目录下所有文件的内容（限小文件，如 < 100KB），这样后续的 Read 大概率命中缓存。
...
- 预取机制减少冷读
- 连接选择就近的 Server 节点
- 对于执行环境来说，一次任务通常读几个到十几个文件，总延迟在可接受范围
...
- 支持分段读取（offset + length），执行环境通常不需要读完整大文件

### 4.3 并发写冲突

**风险：** 执行环境写文件时，本地用户同时也在编辑。

**应对：**
- 小团队场景，冲突概率很低
- 简单方案：最后写入者胜（last-write-wins）
- 可选方案：执行环境写操作前检查文件修改时间，如果和缓存的不一致，提示冲突
...
**风险：** Daemon 网络断开时，执行环境还在执行操作，FUSE 会阻塞。

**应对：**
- FUSE 回调设置超时（如 10 秒），超时返回 EIO 错误
- 缓存的内容在断线期间仍可读取（降级为只读）
- Server 标记用户状态为 offline，执行环境侧可感知
...
执行环境的 bash 命令直接操作 FUSE 挂载的路径，完全透明。

| 维度 | 评估 |
|------|------|
| 执行环境兼容性 | ★★★★★ 完全透明，bash 命令无需任何适配 |
...
Daemon 启动时把文件内容全量同步到 Server 本地磁盘，后续通过 fsnotify 增量同步。执行环境直接操作 Server 本地的真实文件。执行环境写文件后，Server 把变更同步回 Daemon。
...
不用 FUSE，改为拦截执行环境的 bash 命令，解析出文件路径后走 gRPC 获取内容。
...
**当前统一采用方案 A（FUSE 虚拟文件系统）作为主方案。**

理由：

- 它与执行环境的文件访问语义完全一致，路径透明，不需要改造命令执行方式
- Server 不需要维护用户工作区的完整真实副本，避免磁盘镜像带来的一致性和容量问题
- 读写链路可通过缓存、预取和只读降级来控制延迟与可用性
- 架构边界更清晰，Server 负责文件系统抽象与请求转发，Daemon 负责真实文件访问

**方案 B 保留为受限部署环境下的备选降级路径，不作为当前实施方案。**
...
- 响应 Server 下发的读写、目录和属性请求
- fsnotify 监听变更，增量推送

**Server：**
- 接收 Daemon 连接，维护 user_id 到 session 的路由
- 挂载 `/workspace/{user_id}/` FUSE 虚拟文件系统
- 实现 `Lookup`、`Getattr`、`Readdir`、`Open`、`Read`
- 维护文件树缓存和基础内容缓存
- 基本的认证（token）

**此阶段执行环境就可以正常工作了 —— 它操作的是 FUSE 暴露的虚拟工作区。**

### Phase 2：优化（1-2 周）

- 完成 `Write`、`Create`、`Rename`、`Unlink`、`Mkdir`
- 增加内容缓存失效与小文件预取
- 大文件限制与敏感文件过滤
- 断线超时与只读降级
- 多用户隔离与挂载权限控制
...
1. **执行环境归属**：bash 是跑在你控制的 Server 上，还是第三方托管的沙箱里？如果是后者，可能无法直接挂载 FUSE 或落真实文件，方案需要大改。

2. **执行环境工作区路径**：执行 bash 时，它的工作目录（pwd）在哪里？能否配置？这决定了文件同步到 Server 的哪个路径。

3. **执行环境 session 模型**：执行环境是每次任务新建一个沙箱，还是有持久化环境？如果每次新建，文件需要每次重新同步。

4. **写操作频率**：执行环境主要是读文件理解代码，还是会频繁写文件？写操作的频率影响同步策略设计。
