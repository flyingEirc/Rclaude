...
- 统一入口 `rclaude`（daemon + 远端 agent PTY attach）跑在你的本地机器上，
  并反向连接到该 server。

它刻意不是生产交付物：没有 Docker、`systemd`、TLS 或安装器。固定的连接信息来自
`rclaude-remote-local-test` skill：远程访问走 Tailscale SSH（`ssh root@dmit`，
无需密钥材料）；token（以及旧的兜底 SSH key）存放在 `.list/`（已 gitignore）。

## 拓扑

- **Server 机器** —— 远程 Linux，通过 FUSE 挂载 `/workspace/<user_id>/<project>` 并在其中启动
  attach 声明的 agent 程序（如真正的 `claude`），不提供 shell 兜底。
  必须是带 `/dev/fuse` 的 Linux，无法跑在 macOS 上。
- **本地机器** —— **在项目根目录下**运行 `rclaude`：daemon 把该目录暴露给 server
  （目录名即 `<project>`），终端随后 attach 到远端 PTY。会话要运行的 agent 在启动
  命令行声明：`rclaude -g <agent> -c <config>`。客户端一侧不需要 FUSE，
  所以 macOS 也可以。

`RemotePTY.Attach` 只在 `rclaude` 与 server 端 PTY 进程之间传输终端字节/尺寸变更/退出码，
外加声明的 agent 名。`RemoteFS.Connect` + FUSE 暴露 `/workspace/<user_id>/<project>`，
并把文件操作重定向回你的 daemon 工作区。Server 在 `/workspace/<user_id>/<project>` 内
启动 attach 声明的 agent；agent 退出即会话结束，无 shell 兜底，argv 也不受客户端控制。
当你声明 `claude` 时，**server** 机器必须装好它、在 `PATH` 上（或给 `-g` 传 server 上的
绝对路径），并且已经为 server 的 OS 用户登录过 —— 本地的 Claude 登录态不会在 server 侧复用。
...
| `start-rclaude.sh` | 在本地前台运行统一入口 `rclaude`（daemon + pty）：`sh start-rclaude.sh <agent> <config>`。需要交互式 TTY。 |
...
2. 从 `.list/token.yaml` 生成两份 gitignore 的测试配置（已生成过一次；token 轮换后需重新生成）。
   ...
   `daemon.test.yaml` 使用 `server.address: "69.63.208.133:7969"` 和同一个 token。
   没有 `workspace.path` 配置项：工作区根 = 启动 `rclaude` 时所在目录，请在要暴露的
   项目根目录下运行。
...
在一个真实的交互式终端里，声明要运行的 agent：

```sh
sh ./deploy/minimal/start-rclaude.sh claude ./deploy/minimal/daemon.test.yaml
```

（或直接：`./bin/rclaude -g claude -c ./deploy/minimal/daemon.test.yaml`）

预期：先打印 `daemon started`，再打印 `pty started`，然后你落入 `/workspace/<user_id>/<project>`
下的远端 agent PTY 会话。其它一切都写进 `rclaude.log`（默认 `~/.rclaude/logs`）。
...
- SSH 失败：先确认 tailnet 在线（shell 里 `ssh root@dmit` 试一下）。要走裸 IP
  兜底时，设置 `RCLAUDE_SSH_HOST`/`RCLAUDE_SSH_KEY` 并确认
  `.list/server_private_key` 存在（权限 `600`）。
