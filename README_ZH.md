...
pkg/ratelimit/          Server 侧 PTY attach/stdin 限流
...
没有 `pty:` 段，也没有 `log:` 段：PTY 与日志行为都已在代码中写死。PTY 工作目录根
等于 `fuse.mountpoint`（实际 cwd 为 `{mountpoint}/{user_id}/{project}`），环境变量透传
白名单是固定的默认集合（`TERM`、`LANG`、`LC_ALL`、`LC_CTYPE`、`PATH`、`HOME`、`SHELL`、
`CLAUDE_CONFIG_DIR`），PTY 单帧上限为 65536 字节，优雅停止为 5s，attach/stdin 限流使用
固定默认值。日志始终以 JSON 写出全部级别（debug 及以上）到 `~/.rclaude/logs` 并按默认
策略轮转。启动重试固定为 3 次、间隔 5s。
...
Server 侧 PTY 行为不可配置，已在代码中写死：工作目录根等于 `fuse.mountpoint`
（实际 cwd 为 `{mountpoint}/{user_id}/{project}`），环境变量透传白名单是固定的默认
集合（`TERM`、`LANG`、`LC_ALL`、`LC_CTYPE`、`PATH`、`HOME`、`SHELL`、`CLAUDE_CONFIG_DIR`），
PTY 单帧上限为 65536 字节，优雅停止为 5s，attach/stdin 限流使用固定默认值。
...
会轮转的日志文件，保证终端输出是干净透传。日志不可配置：两侧始终以 JSON 写出全部
级别（debug 及以上）到 `~/.rclaude/logs`，并按默认策略轮转。
...
`daemon not connected` 再重试。残余失败仍会回退到事件总线重试，采用固定上限（重试 3 次、间隔 5s）。
