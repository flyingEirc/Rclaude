...
2. 如果是当前阶段要做什么、做到哪一步、怎么验收，先看 `docs/exec-plan/active/`
...
| 当前正在执行的阶段 | `docs/exec-plan/active/{时间}-{阶段名}/` |
...
`docs/exec-plan/` **只承载阶段性执行记录**，不承载系统级 plan。系统级 roadmap 一律放 `docs/design/ROADMAP.md`。

### 阶段目录三件套

每一个开发阶段（一次"从拆分到验收"的完整闭环）必须对应一个目录：
